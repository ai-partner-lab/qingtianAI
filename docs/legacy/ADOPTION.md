...
For a guided browser tour with six animated roles and seven explicit steps, run
`qingtian demo-web`. It uses the real local store, finishes a synthetic Task at
`DONE`, and demonstrates `UNKNOWN` reconciliation and checkpoint handoff. It
requires no model credentials, browser build, or external services. See the
[guided demo](DEMO.md) for loopback-only operation and temporary-data lifetime.
The following older CLI demo remains a smaller primitives-only smoke check:
