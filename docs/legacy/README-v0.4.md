...
## Explore AI responsibilities and capabilities

After installation, launch the local interactive experience with one command:

```bash
qingtian demo-web
```

Your browser opens a loopback-only AI collaboration stage. The chibi illustrations
represent responsibilities, not people or six independently connected models.
Explore the expandable responsibility map: Qingtian → phase Leader → specialist
Agent → concrete work items → checks and artifacts. Click a node to reveal the
next level and inspect its requirements or evidence. Browsing a branch does not
change the live task. Click **Next** through seven
guided steps: planning, scoped knowledge, execution, an injected acknowledgement-loss
scenario, reconciliation and handoff, review, and archiving. Six animated characters
show which role owns each step; the inspectable Task, Session, Run, Evidence, and
Checkpoint records come from the real control-plane store, not a browser mock.

The scenario is deterministic and synthetic. It uses the bundled offline echo
provider, requires no model key, npm build, Docker, account, or external service, and
does not access a real project or Vault. It is a teaching orchestrator, not a
production agent scheduler. Stop with Ctrl+C; the temporary demo data is cleaned up.
Use `qingtian demo-web --port 8788 --no-browser` to select another port or open the
printed URL yourself. See [Demo guide](DEMO.md) and the Chinese
[product introduction](BRAND.md).

The verification phase also runs isolated checks without manually advancing the
guide. API end-to-end verification has no extra runtime dependencies:

```bash
qingtian demo-check --capability api-e2e
```

For real browser E2E, install the optional browser dependency and Chromium once:
...
The UI exposes the same fixed checks. Each uses a fresh synthetic local Demo,
returns an independent receipt, and leaves the guide task unchanged. Missing setup
is `blocked`, never a pass. These checks do not accept a target URL or test a real
business environment. See [Capabilities](CAPABILITIES.md) for the implemented,
adapter-required and planned modules. Leader and Agent nodes describe coordination
responsibilities, not additional processes or model connections. Only real check
receipts can mark a mapped check as passed; planned branches remain planned.
...
boundary. See [Architecture](ARCHITECTURE.md) for the trust model.
...
```bash
python -m pip install './dist/qingtian_ai-0.4.0-py3-none-any.whl[pdf]'
```
...
Read [Adoption](ADOPTION.md) before connecting real projects or providers,
