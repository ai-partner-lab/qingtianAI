...
```text
project adapter                    provider adapter
      |                                  |
      v                                  v
+--------------------+          +----------------------+
| Qingtian control   |          | Qingtian Knowledge  |
| Task / Session /   |          | allowlist / Vault /  |
| Run / Evidence /   |          | provenance / FTS /   |
| Checkpoint / CAS   |          | provider contract    |
+---------+----------+          +----------+-----------+
          |                                |
          v                                v
  private control DB               private knowledge root
          |                                |
          +------ adopter-owned adapter ---+
                     explicit policy
```
...
- **Run** records one bounded execution attempt, executor, idempotency key, request,
...
Verification receipts distinguish observed results from reports and missing evidence.
...
authorized project workspace
             |
             | explicit roots, includes, exclusions, limits
             v
     local source registry
             |
             v
 plan -> extract -> hash -> sensitive-data checks
                           |                |
                    accepted content   restricted match
                           |                |
                           v                v
                 private Markdown     opaque quarantine
                      Vault              metadata
                           |
